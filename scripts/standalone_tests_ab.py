...
log_file = open("reports/forensic_tests_ab_log.txt", "w", encoding="utf-8")
...
try:
    ...
    async def run_test_a():
        log("================================================================================")
        log("TEST A: EXCHANGE ACCEPTANCE -> PROCESS CRASH -> RECONCILIATION")
        log("================================================================================")
        ...
        log(f"Step 1: Claimed execution lock -> execution_id={execution_id}, claimed={claimed}")
        ...
        log(f"Step 2: Exchange accepted order -> exchange_order_id={order_res.exchange_order_id}")
        ...
        log(f"Step 3 [PRE-CRASH STATE]: DB status={pre_crash_rec.status}, order_id={pre_crash_rec.order_id}")
        log(f"Step 3 [EXCHANGE STATE]: Total exchange orders = {len(mock_exchange.orders)} ({mock_exchange.orders[0]['exchange_order_id']})")
        
        # 4. SIMULATE WORKER RESTART & WATCHDOG RECONCILIATION
        log("Step 4: Simulating Process Restart & Watchdog Reconciliation...")
        ...
        # Exact matching algorithm: Query exchange for open/recent orders matching symbol & size
        ...
        log(f"Step 5: Matching algorithm matched exchange_order_id={matching_exchange_order['exchange_order_id']}")
        ...
        log(f"Step 6 [POST-RECOVERY STATE]: DB status={post_rec.status}, order_id={post_rec.order_id}")
        log(f"Step 6 [FINAL COUNTS]: DB Executions=1, Exchange Orders={len(mock_exchange.orders)}")
        
        # 6. RETRY BEHAVIOR
        log("Step 7: Retrying original intent...")
        ...
        log(f"Step 7 [RETRY RESULT]: action={retry_action}, no new exchange submission dispatched.")
        ...
        db_session.close()
        log(">>> TEST A RESULT: PASS (VERIFIED) <<<\n")
        return {
            "execution_id": execution_id,
    ...
    async def run_test_b():
        log("================================================================================")
        log("TEST B: IDENTICAL PARAMETERS BUT TWO INDEPENDENT INTENTS")
        log("================================================================================")
        ...
        log(f"Submitting Intent A (symbol={symbol}, size={size}, price={price}, task={task_id_A})...")
        ...
        log(f"Intent A Result: status={res_A['status']}, execution_id={res_A['execution_id']}")
        
        log(f"Submitting Intent B (symbol={symbol}, size={size}, price={price}, task={task_id_B})...")
        ...
        log(f"Intent B Result: status={res_B['status']}, execution_id={res_B['execution_id']}")
        
        log(f"Current Exchange Orders: {len(mock_exchange.orders)}")
        ...
        log(f"Retry Intent A: status={retry_res_A['status']} (exchange orders count={len(mock_exchange.orders)})")
        ...
        log(f"Retry Intent B: status={retry_res_B['status']} (exchange orders count={len(mock_exchange.orders)})")
        assert retry_res_B['status'] == "skipped_completed", "Retry B must be skipped"
        assert len(mock_exchange.orders) == 2, "Retry must not create new exchange order"
        
        # Concurrent Submissions (Intent A x 10, Intent B x 10)
        log("Executing Concurrent Storm: 10 concurrent requests for Intent A & 10 for Intent B...")
        tasks_A = [
        ...
        log(f"Total concurrent results received: {len(all_results)}")
        log(f"Final Exchange Order Count: {len(mock_exchange.orders)}")
        assert len(mock_exchange.orders) == 2, "Concurrent duplicate storm must NOT create additional exchange orders"
        
        log(">>> TEST B RESULT: PASS (VERIFIED) <<<\n")
        return {
    ...
    async def main():
        ...
        log("ALL FORENSIC PROOFS COMPLETED SUCCESSFULLY!")
