...
if (code.includes('function Leaderboard() {')) {
    console.log("SUCCESS: Leaderboard already exists in the file.");
} else {
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"monospace"}}>
          <thead>
            <tr style={{borderBottom:\`1px solid \${C.border}\`}}>
              {["Rank","Trader","Strategy","P&L","Max DD","Trades","Plan"].map(h=><th key={h} style={{color:C.t3,fontWeight:900,padding:"10px 14px",textAlign:"left",fontSize:8,letterSpacing:2,textTransform:"uppercase"}}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {loading ? (
...
`;
    ...
    // Find the exact injection point: Right above 'function App() {'
    const target = 'function App() {';
    if (code.includes(target)) {
        code = code.replace(target, leaderboardCode + '\\n\\n' + target);
        fs.writeFileSync(file, code, 'utf8');
        console.log("SUCCESS: Forcibly injected the Leaderboard component.");
    } else {
        console.log("FAILED: Could not find 'function App() {' in the file.");
    }
}
