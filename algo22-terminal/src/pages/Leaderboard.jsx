...
import { C, Card, SectionH, PanelTitle } from "../components/ui-legacy/primitives";
            ...
            {loading ? (
              ...
              leaderboardData.map(r => (
                <tr key={r.rank} style={{ borderBottom: `1px solid ${C.border}15`, background: r.rank <= 3 ? `rgba(0,212,255,0.02)` : undefined }} className="hover:bg-white/5 transition-colors">
                  <td style={{ padding: "10px 14px" }}><span style={{ color: r.rank === 1 ? C.gold : r.rank === 2 ? "#c0c0c0" : r.rank === 3 ? "#cd7f32" : C.t3, fontWeight: 900, fontSize: r.rank <= 3 ? 14 : 10 }}>{r.rank === 1 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â¡" : r.rank === 2 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã‹â€ " : r.rank === 3 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â°" : `#${r.rank}`}</span></td>
