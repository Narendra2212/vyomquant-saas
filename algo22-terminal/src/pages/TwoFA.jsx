      ...
      <div style={{ background: token.surface.raised, border: `1px solid ${token.line.default}`, borderRadius: 18, padding: 36, width: "100%", maxWidth: 440 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: "50%", width: 60, height: 60, display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
            <ShieldCheck size={26} style={{ color: token.brand.base }} />
          </div>
          <h1 style={{ color: token.content.primary, fontWeight: 900, fontSize: 20, letterSpacing: "-0.02em" }}>
            {hasExistingFactor ? "Two-Factor Verification" : "Setup Two-Factor Authentication"}
          </h1>
          <p style={{ color: token.content.secondary, fontSize: 11, fontFamily: "monospace", marginTop: 6, lineHeight: 1.5 }}>
            {step === 1
        ...
        {/* Loading Spinner */}
        {isLoading ? (
          <div style={{ padding: "40px 0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, color: token.content.muted, fontFamily: "monospace", fontSize: 11 }}>
            <RefreshCw size={24} className="animate-spin" style={{ color: token.brand.base }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: token.surface.inset, border: `1px solid ${token.line.default}`, borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
              {qrCodeData ? (
                ...
                <div style={{ height: 180, width: 180, background: token.surface.inset, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", color: token.content.muted, fontSize: 10, textAlign: "center", padding: 10 }}>
                  QR code unavailable. Please use the secret key below.
                </div>
              )}
              <span style={{ color: token.content.muted, fontSize: 10, fontFamily: "monospace", textAlign: "center" }}>
            ...
            <div style={{ background: token.surface.inset, border: `1px solid ${token.line.default}`, borderRadius: 10, padding: "12px 14px" }}>
              <div style={{ color: token.content.secondary, fontSize: 10, fontFamily: "monospace", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                <Key size={12} style={{ color: token.brand.base }} />
                <span>Manual Setup Key:</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <code style={{ color: token.brand.base, background: token.surface.panel, flex: 1, padding: "8px 12px", borderRadius: 6, fontSize: 12, fontFamily: "monospace", letterSpacing: "0.1em", userSelect: "all", overflowX: "auto" }}>
          ...
          <form onSubmit={handleVerifyCode} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              {code.map((d, i) => (
                <input
                  ...
                  style={{
                    ...
                    boxShadow: d ? `0 0 10px ${token.brand.base}20` : "none",
                  }}
                />
              ))}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
              {!hasExistingFactor ? (
                <button
                  type="button"
                  style={{ color: token.content.muted, fontSize: 10, fontFamily: "monospace", background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
                  onClick={() => setStep(1)}
              ...
              <button
                type="button"
                style={{ color: token.content.muted, fontSize: 10, fontFamily: "monospace", background: "transparent", border: "none", cursor: "pointer" }}
