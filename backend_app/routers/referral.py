...
async def get_referral_stats(
    ...
    try:
        ...
        logger.error(f"Error fetching referral stats for user {user['id']}: {e}")
...
@router.post("/referral/process-commission")
async def process_referral_commission(
    ...
    This is typically called from payment webhooks.
    """
    if not supabase:
        raise HTTPException(status_code=503, detail="Database connection unavailable")
    
    try:
        # Call database function to process commission
        result = supabase.rpc("process_referral_commission", {
            "p_referred_id": referred_id,
            "p_payment_id": payment_id,
            "p_subscription_tier": subscription_tier,
            "p_payment_amount_usd": payment_amount_usd
        }).execute()
        
        logger.info(f"Commission processed for payment {payment_id}, referred user {referred_id}")
        
        return {"status": "success", "message": "Commission processed"}
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error processing commission for payment {payment_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to process commission")


@router.post("/referral/reverse-commission")
async def reverse_referral_commission(
    payment_id: str,
    reversal_reason: str,
    supabase: SupabaseClient = Depends(get_request_supabase),
):
    """
    Reverse commission for a refund or chargeback.
    This is typically called from payment webhooks.
    """
    if not supabase:
        raise HTTPException(status_code=503, detail="Database connection unavailable")
    ...
    try:
        # Call database function to reverse commission
        result = supabase.rpc("reverse_referral_commission", {
            "p_payment_id": payment_id,
            "p_reversal_reason": reversal_reason
        }).execute()
        
        logger.info(f"Commission reversed for payment {payment_id}, reason: {reversal_reason}")
        ...
        logger.error(f"Error reversing commission for payment {payment_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to reverse commission")
